/*                                   E.Ammann           */
/* Demo program 'w32_pc_mutex.exe' for the Win32 API.   */
/* Application is the producer-consumer problem with    */
/* n producers and 1 consumer.                          */
/* Hier: Nur Garantie des exklusiven Zugriffs auf den   */
/*       Ringpuffer, kein Overflow/Underflow-Schutz     */ 
/*                                                      */
/* file: w32_pc_mutex.cpp                               */
...
HANDLE hSema_mutex;      /* handles to semaphore */
struct fifo_buf buf;
...
void main(int argc, char *argv[])
{
	...
	hSema_mutex = CreateSemaphore(NULL, 1, 1, "mutex_sem");
...
DWORD producer(LPVOID arg)
{
	...
	for (i=0; i<NO_PRODUCTS; i++) {
		...
		WaitForSingleObject(hSema_mutex, INFINITE); /* begin of critical section */
		buf.fifo[buf.in_index] = product;
		if (buf.in_index == FIFO_SIZE - 1) buf.in_index = 0;
				              else buf.in_index++;
		ReleaseSemaphore(hSema_mutex, 1, NULL);     /* end of critical section */
	}
