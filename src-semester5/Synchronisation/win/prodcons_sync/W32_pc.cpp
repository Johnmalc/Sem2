/*                                   E.Ammann  12/1/95  */
/* Demo program 'w32_pc.exe' for the Win32 API.         */
...
HANDLE hSema_mutex, hSema_empty, hSema_full;  /* handles to seamphores */
struct fifo_buf buf;
...
DWORD producer(LPVOID arg)
{
	...
	for (i=0; i<NO_PRODUCTS; i++) {
		...
		WaitForSingleObject(hSema_mutex, INFINITE); /* begin of crit. section*/
		buf.fifo[buf.in_index] = product;
		if (buf.in_index == FIFO_SIZE - 1) buf.in_index = 0;
					      else buf.in_index++;
		ReleaseSemaphore(hSema_mutex, 1, NULL);     /* end of crit. section  */
